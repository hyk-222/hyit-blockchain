...
def transaction_search(request):
    ...
    if request.method == 'GET':
        ...
        for i in output_info:
            sql5 = "SELECT bitcoin_inputs.spending_transaction_hash 'next_hash' from bitcoin_inputs,bitcoin_outputs WHERE bitcoin_outputs.recipient='{}' and bitcoin_outputs.transaction_hash='{}' and bitcoin_inputs.transaction_hash=bitcoin_outputs.transaction_hash".format(i[0], get_hash)
            next_hash_info = mysqlutil.queryOperation(sql5)
            if next_hash_info:
                output_list.append({'address': i[0], 'value': i[1], 'next_hash': next_hash_info[0][0]})
            else:
                output_list.append({'address': i[0], 'value': i[1], 'next_hash': "null"})
