...
function onResize(e, edgeClass, moveAxis) {
    ...
    if (edgeClass == ".edge-right" || edgeClass == ".edge-bottom") {
        // 使用Math.max限定范围
        container.style[moveAxis] = `${Math.min(edgeClass ==
            ".right"?window.innerWidth-left:window.innerHeight-top,Math.max(200,-movement))}px`
    }
    if (edgeClass == ".edge-top" || edgeClass == ".edge-left") {
        let movement = moveAxis == "width" ? left - Math.max(limitX, e.clientX) : top - Math.max(limitY, e.clientY)
        container.style[moveAxis] = `${Math.max(200,moveDirect+movement)}px`
        if (moveAxis == "width") {
